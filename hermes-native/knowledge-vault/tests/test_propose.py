...
class ProposeTests(unittest.TestCase):
    def test_a_proposal_lands_in_the_spool_readable_by_the_review_user(self):
        with tempfile.TemporaryDirectory() as root:
            spool = Path(root)
            proposal = propose("# Hallazgo\nLonghorn usa local-path aca.", {"agent": "jarvis"}, spool)
    ...
    def test_the_same_content_is_not_proposed_twice(self):
        with tempfile.TemporaryDirectory() as root:
            spool = Path(root)
            first = propose("# Igual\nMismo texto.", {"agent": "jarvis"}, spool)
            second = propose("# Igual\nMismo texto.", {"agent": "jarvis"}, spool)
    ...
    def test_what_it_writes_is_what_review_reads(self):
        ...
        with tempfile.TemporaryDirectory() as root:
            ...
            spool.mkdir()
            proposal = propose("# Contrato\nCuerpo de la nota.", {"agent": "jarvis"}, spool)
            projected, _ = run_review(spool, pending, decisions)
            self.assertEqual([pending / f"{proposal.id}.md"], projected)
            self.assertIn("# Contrato", projected[0].read_text(encoding="utf-8"))
    ...
    def test_a_queued_proposal_survives_a_spool_that_is_briefly_unwritable(self):
        with tempfile.TemporaryDirectory() as root:
            ...
            try:
                ...
                proposal = Proposal.create("# Caida\nTexto.", "key", {"agent": "jarvis"})
