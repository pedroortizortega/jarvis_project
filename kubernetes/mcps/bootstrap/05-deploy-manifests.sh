#!/usr/bin/env bash
# Apply the 6 memory-router manifests in dependency order. Secrets referenced
# by name (memory-router-engram-credentials, memory-router-client-bearers,
# memory-router-server-tls, memory-router-client-ca) must already exist —
# run 03-create-secrets.sh first, or the Deployment lands in
